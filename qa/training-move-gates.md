...
Fail if output is only a Question ID. Pass only when target node, move type, conditions, rationale and success criterion are explicit.

## Gate 2 — Observed Profile and desired Requirement are separate
Fail if the engine calls a node “behind” without a relevant target requirement. Current and long-term requirements may coexist without grade staging.
...
Null prerequisite state must not automatically block all downstream work.
...
Pass when a shared evidence-reasoning bottleneck outranks an isolated task-specific weakness because it unlocks multiple core targets. Fail if ranking is simply lowest M first.
...
## Gate 10 — School relevance is normally a tie-breaker/preference
Fail if current school unit becomes the global training path. Comparable canonical moves may prefer timely school material.
...
Every move has an explicit stopping condition. Fail vague criteria such as “练熟一点”.
...
A selected move should answer: current gap/uncertainty; why the node matters; why this move shape/support/complexity; how completion will be judged.

## Gate 31 — Maintenance can escalate when it becomes causally important
A review-due node is normally maintenance. If recent regression on a core prerequisite blocks active downstream performance, the move may escalate to P1/P0.

Fail if all review work is permanently trapped below development work regardless of consequence.

## Gate 32 — School deadline can raise priority only through an explicit requirement
A near-term school assessment may temporarily raise a canonical move when represented as an ActiveRequirement.

Fail if grade/unit identity alone raises priority.

## Gate 33 — Recent saturation lowers marginal value
Once the current evidence purpose is satisfied, another same-form item should not stay high priority merely because the node is globally important.

## Gate 34 — Primary recommendation should not thrash
Keep the move until its evidence purpose is satisfied/invalidated or a materially higher-value blocker appears.
...
- overdue core maintenance with demonstrated regression -> priority escalation;
- explicit school deadline -> temporary priority increase without grade-based progression.
